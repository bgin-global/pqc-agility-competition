...
*A note for later, not a design.* When the calibration tracks have a hosted site, a submitter's runtime — the harness instance that proposed — could appear as a star seated by its own content-derived address, whose lit points are the testbed runs it has been signed on: each node attestation over a canonical result is a signature the star captures and shares with the submission it belongs to, and a track's sky would then be its promoted history as a constellation. The same view could scale a run by the signature sizes it measured (M-01, M-06), which is the one picture the committee will be asked for in every room. The instrument that renders keys this way exists (the sovereignty-lattice star at soulbis.com/star and its sky of many keys; the harness emits the seating as data, `tools/star.mjs`, format `star.v1`) and its own rule applies here unchanged: colour and geometry, never proof — the attestation stays the evidence, the star only shows where it landed. Nothing in this plan depends on it.
