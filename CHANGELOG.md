...
### Added
- Resources: the PQC Audit Index (secondary, primaries linked) — FIPS 206 status with its primary, LMS sizes corrected, the on-ramp round-3 candidates named as out of scope by G1, M-19 checklist seeds (six sections plus per-scheme audit-focus items), the five published migration timelines for governance fit, and the ACVP / CAVP wording of the T1 correctness gate.
- `harness/`: the calibration harness as a dual-agent instance in the record — `upstream/` vendors the agentprivacy dual-agent harness default distribution at a pinned commit (`UPSTREAM.json`, sha256 per file; `scripts/sync-harness.sh` = harness-only sync, `--verify`), `instance/` is the pqc-calibration instance (config with the fixture draw as the Gap, `tools/measure.mjs`, ledgers, a stub round on record verified by `verify_run`); CI runs `scripts/ci/check-harness.sh`. Two upstream defects found on the first round (biased draw for large N; short expansion counter) are recorded and reported.
- `testbed/tracks/` (proposed): the calibration-track contract, the T1 `verify-single` draft (manifest, rules, runner spec, fixtures README), and the submission-note template; result schema gains `manifest.runner`, `manifest.draw`, `track`, `scalar`; the register gains proposed directions; the COI policy gains the measurement-seat and attribution clause; HARNESS §5 gains the promotion rule and the draw; CI validates track manifests.
