...
| CR-H10 | Same-model seat pairs reduce defect detection vs cross-model (Φ_inference) | OPEN | nothing | the reference runtime runs both seats as one model (Φ_inference = 0 by the PVM); a testable, pre-registered prediction; externally motivated by correlated-error and perplexity-driven self-preference findings (`SOURCES.md`: huang-self-correct · wataoka-self-preference, REPORTED) | OPEN — settled by the cross-model delta run (C6) |
