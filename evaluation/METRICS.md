...
| M-19 | Security review score | checklist result (assumptions, parameter set, side channels, stateful-key handling for XMSS / LMS-class schemes) | a | all | Track (a) written review. Checklist seeds (to be adopted at W2): the six sections of the [PQC Audit Index audit checklist](https://pqaudit.org/audit-checklist/) — specification conformance (final FIPS 203/204/205 or SP 800-208; ACVP/CAVP known-answer tests per parameter set; domain separation and context strings), input validation, side channels (no secret-dependent branches or memory access; rejection-sampling loops; tooling such as ctgrind / dudect / formal methods), randomness and hedging (hedged signing unless documented; fault countermeasures), protocol integration (hybrid combiners bind both parts; no silent downgrade), crypto-agility and operations (negotiated or versioned algorithm identifiers; size budgets; a migration plan aligned to a published timeline) — plus the per-scheme audit-focus items the index carries (ML-DSA rejection-sampling leakage; FN-DSA's floating-point Gaussian sampler; LMS / XMSS state persistence, rollback, backup, HSM cloning) | candidate |
